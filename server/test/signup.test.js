...
describe('Sign up API test', () => {
  ...
  it('should throw error if email not passed', (done) => {
    requestBody.email = '';
    request(server).post('/signup').send(requestBody).expect(400, done);
  });
  ...
  it('should throw error if email is existing', (done) => {
    ...
    request(server).post('/signup').send(requestBody).expect(400)
      .end((err) => {
  ...
  it('should save user if evrything is correct', (done) => {
    ...
    const save = stub(User.prototype, 'save').returns(null);
    request(server).post('/signup').send(requestBody).expect(200)
      .end((err) => {
        if (err) return done(err);
        findOneUser.restore();
        save.restore();
